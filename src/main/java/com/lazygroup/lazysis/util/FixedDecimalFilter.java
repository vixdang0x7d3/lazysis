...
public class FixedDecimalFilter implements UnaryOperator<TextFormatter.Change> {
	...
	public TextFormatter.Change apply(TextFormatter.Change change) {
		...
		if ((change.getSelection().getStart() == 0)
				&& (change.getSelection().getEnd() == change.getControlText().length())) {
			...
			if (caretPos > decimalPos) {
				...
				if (decimalSize > decimalPlaces) {
					...
					if (caretPos == newText.length()) {
						change.setText(newText.substring(decimalPos + 2,
								decimalPos + 2 + decimalPlaces));
					} else {
						change.setText(newText.substring(decimalPos + 1,
								decimalPos + 1 + decimalPlaces));
					}
				}
			}
		}
